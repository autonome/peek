...
**3. Sync changes back to agent-workflow repo** (only if you have new learnings)
- **Learnings**: Append new tips/gotchas to `~/sync/Dev/agent-workflow/CLAUDE.md` using Edit tool (don't overwrite - other agents may have added content)
- **Permissions**: Skip - coordinator will consolidate permissions manually
...
## Your Taske2e sync test: desktop + server in debug and then in prod, verify via railway logs for production package
